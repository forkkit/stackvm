...
func TestMach_utf8(t *testing.T) {
	TestCase{
		Name: "rot-13 check",
		Prog: []interface{}{
			// TODO: make this a .in so that .ins can be .outs too
			".data",
			".out", "s:", ".string", "hello world",

			".entry", "main:",
			":s", "fetch", // n :
			4, ":s", "add", "p2c", // : &s[len(s)]
			4, ":s", "cpush", // : &s[i=0] &s[len(s)]
			"loop:",             // : &s[i] &s[len(s)]
			"c2p", "dup", "p2c", // &s[i] : &s[i] &s[len(s)]
			"utf8dec",      // r n : &s[i] &s[len(s)]
			"dup", 1, "hz", // r n : &s[i] &s[len(s)]
			"swap",                             // n r : ...
			"dup", 0x80, "gte", ":next", "jnz", // n r : ...
			"dup", 0x20, "bitor", // n r rl : ...
			"dup", 0x61, "gte", // n r rl rl>='a' : ...
			2, "dup", 0x6d, "lte", // n r rl rl>='a' rl<='m' : ...
			"and",           // n r rl rl>='a'&&rl<='m' : ...
			":check2", "jz", // n r rl : ...
			"pop",     // n r : ...
			13, "add", // n r+=13 : ...
			":next", "jz", // n r : ...
			"check2:",          // n r rl : ...
			"dup", 0x6e, "gte", // n r rl rl>='n' : ...
			"swap", 0x7a, "lte", // n r rl>='n' rl<='z' : ...
			"and",         // n r rl>='n'&&rl<='z' : ...
			":next", "jz", // n r : ...
			13, "sub", // n r-=13 : ...
			"next:",             // n r : ...
			"c2p", "dup", "p2c", // n r &s[i] : &s[i] &s[len(s)]
			"utf8enc",      // n m : &s[i] &s[len(s)]
			2, "dup", "eq", // n m==n : &s[i] &s[len(s)]
			2, "hz", // n : &s[i] &s[len(s)]
			"c2p", "add", // &s[i+n] : &s[len(s)]
			"c2p",    // &s[i+n] &s[len(s)] :
			2, "dup", // &s[i+n] &s[len(s)] &s[i+n] :
			2, "dup", // &s[i+n] &s[len(s)] &s[i+n] &s[len(s)] :
			2, "p2c", // &s[i+n] &s[len(s)] : &s[i+n] &s[len(s)]
			"lt",           // &s[i+n]<&s[len(s)] : &s[i+n] &s[len(s)]
			":loop", "jnz", // : &s[i+n] &s[len(s)]
			2, "cpop", // :
			"halt", // : normal halt
		},
	}.Run(t)
